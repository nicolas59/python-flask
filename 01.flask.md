...
**flask** est un microframework permettant le developpement d'applications Web avec Python. 

Trés facile d'apprentissage, il offre le strict nécessaire pour :
* receptionner des requetes
...
Il est possible de lancer une applicationflask sans avoir recours à la méthode run. Lors de l'installation de Flask, un executable flask a été ajouté à votre path.

> Le fichier *app.py* ne doit pas contenir d'appel à la méthode run.
...
Pour modifier la port, le paramétre **--port** doit être passé en paramétre
